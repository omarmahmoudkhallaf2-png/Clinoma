...
async function run() {
  const email = "temp_agent_3@clinoma.com";
  const password = "password12345";
  
  try {
    console.log("Trying to sign in with existing temp user...");
    await signInWithEmailAndPassword(auth, email, password);
    console.log("Signed in successfully!");
  } catch (err) {
    console.log("Existing user sign-in failed, trying to sign up...");
    try {
      await createUserWithEmailAndPassword(auth, email, password);
      console.log("Registered new temp user successfully!");
    } catch (signupErr) {
      console.error("Sign-up also failed:", signupErr.message);
      process.exit(1);
    }
  }
